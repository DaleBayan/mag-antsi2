...
<div class="row">
    ...
    <div class="p-3 col-6 card border-top border-4 border-success">
        <div class="card-body">
            ...
            <!-- Vertical Form -->
            <form method="POST" action="{{ route('contacts.update', Crypt::encryptString($contact->id)) }}" class="row gy-3 gx-3">
                @csrf
                @method('PUT')
                <div class="col-12">
                    <label for="inputNanme4" class="form-label">Full Name</label>
                    <input type="text" class="form-control" name="full_name" value="{{ $contact->full_name }}">
                    @error('full_name')
                ...
                <div class="col-12">
                    ...
                    <input type="tel" class="form-control" name="mobile_number" pattern="[0]{1}[0-9]{10}" value="{{ $contact->mobile_number }}">
                    @error('mobile_number')
                    <small class="text-danger">{{$message}}</small>
                    @enderror
                </div>
